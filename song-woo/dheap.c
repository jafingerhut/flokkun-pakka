...
void dheap::insert(item i, keytyp k) {
    if ((i < 1) || (i > N)) {
        fprintf(stderr, "dheap::insert(i=%d, k=%lf) Attempting to insert item outside of range [1..N] for N=%d in dheap\n",
                i, k, N);
        abort();
        exit(1);
    }
...
// Change the key of i and restore heap order.
    if ((i < 1) || (i > N)) {
        fprintf(stderr, "dheap::changekey(i=%d, k=%lf) Attempting to change key of item outside of range [1..N] for N=%d in dheap\n",
                i, k, N);
        abort();
        exit(1);
    }
...
void dheap::print() {
	...
	printf("\nkvec:");
	for (x = 1; x <= n; x++) printf(" %8lf",kvec[h[x]]);
	printf("\n pos:");
