...
int main(int argc, char* argv[]){
  ...
  if(fpt != NULL){
    ...
    while(fscanf(fpt,"%u %u %d %d %d %d\n", &header[0], &header[1], &header[2], &header[3], &header[4], &fid) != Null){
      ...
      if(tuplecount == 0)printf("packet %d has no tuple to search\n", index, tuplecount);
      else if(match == 0)printf("packet %d matches no rule, should be %d\n", index, fid);
      else if(matchid != fid) printf("packet %d matches rule %d, should be %d\n", index, exprule[matchid].ruleid, fid);
    }
    printf("check %d tuples at most\n", maxcount);
    printf("Avg accessed tuples %f\n", (float)totalcount/index);  
  }       
  if(done == 1)printf("Classification done for %d packets!\n", index);  
