...
class bst {
    ...
    void init_freelist();
    int alloc_node(int dbg_val);
    int *find_candidate_rules(int numrules, struct pc_rule *rule, int dim, unsigned long mink, unsigned long maxk, int *out_num_rules, std::bitset<MAXRULES> *inout_bitvec);

public:
    bst(int, struct pc_rule*, int);
    ~bst();
